...
export const WORKFLOW_SYSTEM_PROMPT = [
    ...
    "### 4. 验证 — 先运行 npm run typecheck 确保编译通过，再运行 raccoon_run_test 确保测试通过",
    ...
    "### 8. 代码审核 — 用 raccoon_pr_review 获取 PR Diff，检查代码质量、逻辑正确性、边界条件",
    "### 9. 合并 PR — 审核通过且 CI 全绿后，用 raccoon_pr_merge 合并（默认 squash）",
    "",
    "重要原则：",
